...
public class PDF417UtilitiesTest {
    ...
    public void testSymbolSizeCalculation() throws Exception {
        int k;
        k = PDF417ErrorCorrection.getErrorCorrectionCodewordCount(ErrorCorrectionLevel.DEFAULT_ERROR_CORRECTION_LEVEL);
        assertEquals(2, k);
        k = PDF417ErrorCorrection.getErrorCorrectionCodewordCount(ErrorCorrectionLevel.LEVEL_1);
        assertEquals(4, k);
        k = PDF417ErrorCorrection.getErrorCorrectionCodewordCount(ErrorCorrectionLevel.LEVEL_8);
        assertEquals(512, k);

        //What follows is the example in 4.9.2 in the ISO spec.
        ErrorCorrectionLevel ecl = ErrorCorrectionLevel.LEVEL_4;
